...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        ...
        $picsName = $_FILES["image"]["name"];
        $picsType = $_FILES["image"]["type"];
        $picsSize = $_FILES["image"]["size"];
        $maxSize = 5000000;
        // (<= 5Mo)
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $picsType = finfo_file($finfo, $_FILES["image"]["tmp_name"]);
            finfo_close($finfo);
        ...
        if (in_array($picsType, $format)){
            if ($picsSize > $maxSize){
                ...
                move_uploaded_file($_FILES["image"]["tmp_name"], "assets/images_the_district/food" . $_FILES["image"]["name"]);
